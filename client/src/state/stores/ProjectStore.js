...
class ProjectStore {
  ...
  async getAll(
    ...
    try {
      const response = await fetch(
        `${SERVER}/api/users/${state.user.data.id}/projects?pageSize=${
          pageSize || ""
        }&pageNumber=${pageNumber === "" ? 0 : pageNumber}&filterField=${
          filterField || ""
        }&filterValue=${filterValue || ""}&sortField=${
          sortField || ""
        }&sortOrder=${sortOrder || ""}`,
        {
          headers: {
  ...
  async createOne(state, project) {
    try {
      const response = await fetch(
        `${SERVER}/api/users/${state.user.data.id}/projects`,
        {
          method: "post",
          headers: {
  ...
  async updateOne(state, id, project) {
    try {
      const response = await fetch(
        `${SERVER}/api/users/${state.user.data.id}/projects/${id}`,
        {
          method: "put",
          headers: {
            "Content-Type": "application/json",
